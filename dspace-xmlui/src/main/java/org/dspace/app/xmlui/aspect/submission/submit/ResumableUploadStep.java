...
public class ResumableUploadStep extends UploadStep {
	...
	UIException, SQLException, IOException, AuthorizeException
	{
		...
		if (!disableFileEditing){
			...
			Table summary = uploadDiv.addTable("resumable-upload-summary",(bitstreams.size() * 2) + 2,7);
			...
			for (Bitstream bitstream : bitstreams)
			{
				...
				info.addHidden("file-extra-bytes-" + id).setValue(String.valueOf(bitstream.getSizeBytes()));
				info.addHidden("file-extra-format-" + id).setValue(bitstreamService.getFormatDescription(context, bitstream));
			...
			data.addHidden("item-space").setValue(Long.toString(Math.max(max - getItemSizeBytes(item), 0)));
	...
	private long getItemSizeBytes(Item item) {
		long size = 0;
		if(item != null ) {
