 ...
 * document.</li>
 * <li><code>ignoreHandle</code> -- true/false, ignore AIP's idea of handle when
 * ingesting.</li>
 * <li><code>ignoreParent</code> -- true/false, ignore AIP's idea of parent when
 * ingesting.</li>
 * </ul>
 * <p>
 ...
 * <em>false</em> (the default), any existing collection template is
 * ignored.</li>
 * </ul>
 * 
 * @author Larry Stone
...
public abstract class AbstractMETSIngester extends AbstractPackageIngester {
	 ...
	 * contents of an external metadata stream referenced by an <code>mdRef</code>
	 * element in a Zipped up METS manifest.
	 * <p>
	 * Initialize it with the Content (ORIGINAL) Bundle containing all of the
	 * metadata bitstreams. Match an mdRef by finding the bitstream with the same
	 * name.
	 */
	protected static final class MdrefManager implements METSManifest.Mdref {
		...
		/**
		 * Make the contents of an external resource mentioned in an <code>mdRef</code>
		 * element available as an <code>InputStream</code>. See the
		 * <code>METSManifest.MdRef</code> interface for details.
		 * 
	...
	/**
	 * Create a new DSpace object out of a METS content package. All contents are
	 * dictated by the METS manifest. Package is a ZIP archive (or optionally bare
	 * manifest XML document). In a Zip, all files relative to top level and the
	 * manifest (as per spec) in mets.xml.
	 * 
	 ...
	 * 
	 * @throws PackageValidationException if package validation error if package is
	 *                                    unacceptable or there is a fatal error
	 *                                    turning it into a DSpaceObject.
	...
	public DSpaceObject ingest(Context context, DSpaceObject parent, File pkgFile, PackageParameters params,
			String license) throws PackageValidationException, CrosswalkException, AuthorizeException, SQLException,
		...
		try {
			...
			if (manifest == null) {
				throw new PackageValidationException("No METS Manifest found (filename=" + METSManifest.MANIFEST_FILE
			...
			if (!params.restoreModeEnabled() && !params.containsKey("ignoreHandle")) { // ignore the handle in the
																						// manifest, and instead create
																						// a new
																						// handle
			...
			if (parent != null && !params.containsKey("ignoreParent")) { // ignore the parent in the manifest, and
																			// instead use the
			...
			if (dso != null) {
				...
				log.info(LogManager.getHeader(context, action,
						"Created new Object, type=" + Constants.typeText[dso.getType()] + ", handle=" + dso.getHandle()
	...
	/**
	 * Parse a given input package, ultimately returning the METS manifest out of
	 * the package. METS manifest is assumed to be a file named 'mets.xml'
	 * 
	 * @param context DSpace Context
	 ...
	 */
	protected DSpaceObject ingestObject(Context context, DSpaceObject parent, METSManifest manifest, File pkgFile,
			PackageParameters params, String license) throws IOException, SQLException, AuthorizeException,
			CrosswalkException, PackageValidationException, WorkflowException {
		...
		if (type != Constants.SITE && (parent == null || !params.getBooleanProperty("ignoreParent", false))) {
			try {
				...
				// If user specified to skip item ingest if any "missing parent" error message
				// occur
				if (params.getBooleanProperty("skipIfParentMissing", false)) {
					// log a warning instead of throwing an error
					log.warn(LogManager.getHeader(context, "package_ingest", "SKIPPING ingest of object '"
							+ manifest.getObjID() + "' as parent DSpace Object could not be found. "
		...
		if (dso == null) {
			throw new PackageValidationException("Unable to initialize object specified by package (type='" + type
					+ "', handle='" + handle + "' and parent='" + parent.getHandle() + "').");
		}
		...
		if (type == Constants.ITEM) {
			...
			// Do nothing by default -- Crosswalks will handle anything necessary to ingest
			// at Site-level
	...
	/**
	 * Replace the contents of a single DSpace Object, based on the associated METS
	...
	protected DSpaceObject replaceObject(Context context, DSpaceObject dso, METSManifest manifest, File pkgFile,
		...
		if (log.isDebugEnabled()) {
			log.debug("Object to be replaced (handle=" + dso.getHandle() + ") is " + Constants.typeText[dso.getType()]
					+ " id=" + dso.getID());
		}
		...
		if (dso.getType() == Constants.ITEM) {
			...
			// Do nothing -- Crosswalks will handle anything necessary to replace at
			// Site-level
		}
	 ...
	 * @param pkgFile       the full package file (which may include content files
	 *                      if a zip)
	 * @param params        Ingestion Parameters
	...
	protected void addBitstreams(Context context, Item item, METSManifest manifest, File pkgFile,
			PackageParameters params, MdrefManager mdRefCallback) throws SQLException, IOException, AuthorizeException,
			MetadataValidationException, CrosswalkException, PackageValidationException {
		...
		if (primaryID != null && !setPrimaryBitstream) {
			log.warn("Could not find primary bitstream file ID=\"" + primaryID + "\" in manifest file \""
					+ pkgFile.getAbsolutePath() + "\"");
		}
	}

	/**
	 * Save/Preserve the METS Manifest as a Bitstream attached to the given DSpace
	 * item.
	 * 
	 * @param context  DSpace Context
	 * @param item     DSpace Item
	...
	protected void addManifestBitstream(Context context, Item item, METSManifest manifest)
		...
		BitstreamFormat manifestFormat = PackageUtils.findOrCreateBitstreamFormat(context, fmtName, "application/xml",
	...
	protected void addContainerLogo(Context context, DSpaceObject dso, METSManifest manifest, File pkgFile,
			PackageParameters params) throws SQLException, IOException, AuthorizeException, MetadataValidationException,
	 ...
	 * @throws PackageValidationException  if package validation error
	 */
	protected void addTemplateItem(Context context, DSpaceObject dso, METSManifest manifest, File pkgFile,
			PackageParameters params, MdrefManager callback)
			throws SQLException, IOException, AuthorizeException, CrosswalkException, PackageValidationException {
		...
		if (childObjList != null && !childObjList.isEmpty()) {
			...
			if (templateItemDiv != null) {
				...
				if (templateDmdIds != null) {
					...
					// get a reference to the dmdSecs which describe the metadata for this template
					// item
					Element[] templateDmds = manifest.getDmdElements(templateDmdIds);

					// Run our Descriptive metadata (dublin core, etc) crosswalks to add metadata to
					// template item
	 ...
	 * All contents are dictated by the METS manifest. Package is a ZIP archive (or
	 * optionally bare manifest XML document). In a Zip, all files relative to top
	 * level and the manifest (as per spec) in mets.xml.
	 * <P>
	 * This method is similar to ingest(), except that if the object already exists
	 * in DSpace, it is emptied of files and metadata. The METS-based package is
	 * then used to ingest new values for these.
	 * 
	 * @param context      DSpace Context
	 ...
	 * @throws PackageValidationException if package validation error if package is
	 *                                    unacceptable or there is a fatal error
	 *                                    turning it into a DSpace Object.
	 * @throws IOException                if IO error
	 * @throws SQLException               if database error
	 * @throws AuthorizeException         if authorization error
	 * @throws CrosswalkException         if crosswalk error
	...
	public DSpaceObject replace(Context context, DSpaceObject dsoToReplace, File pkgFile, PackageParameters params)
		...
		try {
			...
			if (dsoToReplace == null) {
				...
				if (dso != null) {
					...
					log.info(LogManager.getHeader(context, "package_replace",
							"Created new Object, type=" + Constants.typeText[dso.getType()] + ", handle="
									+ dso.getHandle() + ", dbID=" + String.valueOf(dso.getID())));
				}
			} else
			...
			// it!
			{
				...
				log.info(LogManager.getHeader(context, "package_replace",
						"Replaced Object, type=" + Constants.typeText[dso.getType()] + ", handle=" + dso.getHandle()
								+ ", dbID=" + String.valueOf(dso.getID())));
			}
	 ...
	 * <p>
	 * This is a default implementation which assumes the parent will be specified
	 * in a &lt;structMap LABEL="Parent"&gt;. You should override this method if
	 * your METS manifest specifies the parent object in another location.
	 * 
	 * @param context  DSpace Context
	 * @param manifest METS manifest
	 * @return a DSpace Object which is the parent (or null, if not found)
	 * @throws PackageValidationException  if package validation error if parent
	 *                                     reference cannot be found in manifest
	 * @throws MetadataValidationException if metadata validation error
	 * @throws SQLException                if database error
	 */
	 ...
	 * Object can be found in the &lt;mets&gt; @OBJID attribute. You should override
	 * this method if your METS manifest specifies the handle in another location.
	 * 
	 * If no handle was found then null is returned.
	 * 
	 * @param manifest METS manifest
	 * @return handle as a string (or null, if not found)
	 * @throws PackageValidationException  if package validation error if handle
	 *                                     cannot be found in manifest
	 * @throws MetadataValidationException if validation error
	 * @throws SQLException                if database error
	 */
	...
	/**
	 * Retrieve the inputStream for a File referenced from a specific path within a
	 * METS package.
	 * <p>
	 ...
	 * <p>
	 * Otherwise, the pkgFile is a Zip, so the file should be retrieved from within
	 * that Zip package.
	 * 
	 * @param pkgFile the full package file (which may include content files if a
	 *                zip)
	...
	/**
	 * Returns a user help string which should describe the additional valid
	 * command-line options that this packager implementation will accept when using
	 * the <code>-o</code> or <code>--option</code> flags with the Packager script.
	 *
	 * @return a string describing additional command-line options available with
	 *         this packager
	 */
	@Override
	...
	/**
	 * Profile-specific tests to validate manifest. The implementation can access
	 * the METS document through the <code>manifest</code> variable, an instance of
	 * <code>METSManifest</code>.
	 * 
	 * @throws MetadataValidationException if metadata validation error if there is
	 *                                     a fatal problem with the METS document's
	 ...
	 * ingested. It is responsible for calling the crosswalk, using the manifest's
	 * helper i.e.
	 * <code>manifest.crosswalkItemDmd(context,item,dmdElement,callback);</code>
	 * (The <code>callback</code> argument is a reference to itself since the class
	 * also implements the <code>METSManifest.MdRef</code> interface to fetch
	 * package files referenced by mdRef elements.)
	 * <p>
	 ...
	 * implementation of this method controls exactly what licenses are added to the
	 * new item, including the DSpace deposit license. It is given the collection
	 * (which is the source of a default deposit license), an optional user-supplied
	 * deposit license (in the form of a String), and the METS manifest. It should
	 * invoke <code>manifest.getItemRightsMD()</code> to get an array of
	 * <code>rightsMd</code> elements which might contain other license information
	 * of interest, e.g. a Creative Commons license.
	 * <p>
	 * This framework does not add any licenses by default.
	 * <p>
	 * Note that crosswalking rightsMD sections can also add a deposit or CC license
	 * to the object.
	 * 
	 * @param context    the DSpace context
	 * @param item       Item
	...
	public abstract void addLicense(Context context, Item item, String license, Collection collection,
			PackageParameters params) throws PackageValidationException, AuthorizeException, SQLException, IOException;

	/**
	 * Hook for final "finishing" operations on the new Object. This method is
	 * called when the new Object is otherwise complete and ready to be returned.
	 * The implementation should use this opportunity to make whatever final checks
	...
	/**
	 * Subclass-dependent final processing on a Bitstream; could include fixing up
	 * the name, bundle, other attributes.
	 * 
	 * @param context  context
	 * @param manifest METS manifest
	 * @param bs       bitstream
	...
	public abstract void finishBitstream(Context context, Bitstream bs, Element mfile, METSManifest manifest,
			PackageParameters params) throws MetadataValidationException, SQLException, AuthorizeException, IOException;

	/**
	 * Returns keyword that makes the configuration keys of this subclass unique,
	 * e.g. if it returns NAME, the key would be: "mets.NAME.ingest.preserveManifest
	 * = true"
	 * 
	 * @return name
	 */
