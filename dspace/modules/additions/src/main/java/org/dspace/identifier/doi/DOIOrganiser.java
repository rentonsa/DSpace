...
public class DOIOrganiser {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("Cannot save changes to database: " + sqle.getMessage());
			System.exit(-1);
		}
	...
	public static void runCLI(Context context, DOIOrganiser organiser, String[] args) {
		...
		Option registerDoi = OptionBuilder.withArgName("DOI|ItemID|handle").withLongOpt("register-doi").hasArgs(1)
		...
		Option reserveDoi = OptionBuilder.withArgName("DOI|ItemID|handle").withLongOpt("reserve-doi").hasArgs(1)
		...
		Option update = OptionBuilder.withArgName("DOI|ItemID|handle").hasArgs(1)
				...
				.withLongOpt("update-doi").create();

		options.addOption(update);

		Option delete = OptionBuilder.withArgName("DOI identifier").withLongOpt("delete-doi").hasArgs(1)
				.withDescription("Delete a specified identifier.").create();
		...
		if (line.hasOption('l')) {
			...
			organiser.list("update", null, null, DOIIdentifierProvider.UPDATE_BEFORE_REGISTRATION,
					DOIIdentifierProvider.UPDATE_REGISTERED, DOIIdentifierProvider.UPDATE_RESERVED);
		...
		if (line.hasOption('s')) {
			...
			try {
				List<DOI> dois = doiService.getDOIsByStatus(context,
						Arrays.asList(DOIIdentifierProvider.TO_BE_RESERVED));
				if (0 == dois.size()) {
					System.out.println("There are no objects in the database " + "that could be reserved.");
				}
		...
		if (line.hasOption('r')) {
			...
			try {
				...
				if (0 == dois.size()) {
					System.out.println("There are no objects in the database " + "that could be registered.");
				}
				for (DOI doi : dois) {
		...
		if (line.hasOption('u')) {
			...
			try {
				List<DOI> dois = doiService.getDOIsByStatus(context,
						Arrays.asList(DOIIdentifierProvider.UPDATE_BEFORE_REGISTRATION,
								DOIIdentifierProvider.UPDATE_RESERVED, DOIIdentifierProvider.UPDATE_REGISTERED));
				if (0 == dois.size()) {
					System.out.println("There are no objects in the database " + "whose metadata needs an update.");
				}
		...
		if (line.hasOption('d')) {
			...
			try {
				List<DOI> dois = doiService.getDOIsByStatus(context,
						Arrays.asList(DOIIdentifierProvider.TO_BE_DELETED));
				if (0 == dois.size()) {
					System.out.println("There are no objects in the database " + "that could be deleted.");
				}
	...
	public void register(DOI doiRow) throws SQLException {
		...
		try {
			provider.registerOnline(context, dso, DOI.SCHEME + doiRow.getDoi());

			if (!quiet) {
				System.out.println("This identifier: " + DOI.SCHEME + doiRow.getDoi() + " is successfully registered.");
			}
		} catch (IdentifierException ex) {
			if (!(ex instanceof DOIIdentifierException)) {
				LOG.error(
						"It wasn't possible to register this identifier: " + DOI.SCHEME + doiRow.getDoi() + " online. ",
						ex);
			}
			...
			try {
				sendAlertMail("Register", dso, DOI.SCHEME + doiRow.getDoi(),
						doiIdentifierException.codeToString(doiIdentifierException.getCode()));
			} catch (IOException ioe) {
				LOG.error("Couldn't send mail", ioe);
			}

			LOG.error("It wasn't possible to register this identifier : " + DOI.SCHEME + doiRow.getDoi()
					+ " online. Exceptions code: "
					+ doiIdentifierException.codeToString(doiIdentifierException.getCode()), ex);

			if (!quiet) {
				System.out.println("It wasn't possible to register this identifier: " + DOI.SCHEME + doiRow.getDoi());
			}

		} catch (IllegalArgumentException ex) {
			LOG.error("Database table DOI contains a DOI that is not valid: " + DOI.SCHEME + doiRow.getDoi() + "!", ex);
			...
			throw new IllegalStateException(
					"Database table DOI contains a DOI " + " that is not valid: " + DOI.SCHEME + doiRow.getDoi() + "!",
					ex);
		} catch (SQLException ex) {
	...
	public void reserve(DOI doiRow) throws SQLException {
		...
		try {
			provider.reserveOnline(context, dso, DOI.SCHEME + doiRow.getDoi());
			...
			try {
				sendAlertMail("Reserve", dso, DOI.SCHEME + doiRow.getDoi(),
						DOIIdentifierException.codeToString(doiIdentifierException.getCode()));
			} catch (IOException ioe) {
				LOG.error("Couldn't send mail", ioe);
			}

			LOG.error("It wasn't possible to reserve the identifier online. " + " Exceptions code:  "
					+ DOIIdentifierException.codeToString(doiIdentifierException.getCode()), ex);

			if (!quiet) {
				System.out.println("It wasn't possible to reserve this identifier: " + DOI.SCHEME + doiRow.getDoi());
			}
		} catch (IllegalArgumentException ex) {
			LOG.error("Database table DOI contains a DOI that is not valid: " + DOI.SCHEME + doiRow.getDoi() + "!", ex);
	...
	public void update(DOI doiRow) {
		...
		try {
			provider.updateMetadataOnline(context, dso, DOI.SCHEME + doiRow.getDoi());

			if (!quiet) {
				System.out.println("Successfully updated metadata of DOI " + DOI.SCHEME + doiRow.getDoi() + ".");
			}
		} catch (IdentifierException ex) {
			if (!(ex instanceof DOIIdentifierException)) {
			...
			try {
				sendAlertMail("Update", dso, DOI.SCHEME + doiRow.getDoi(),
						doiIdentifierException.codeToString(doiIdentifierException.getCode()));
			} catch (IOException ioe) {
				LOG.error("Couldn't send mail", ioe);
			}
			...
			LOG.error("It wasn't possible to update this identifier:  " + DOI.SCHEME + doiRow.getDoi()
					+ " Exceptions code:  " + doiIdentifierException.codeToString(doiIdentifierException.getCode()),
					ex);

			if (!quiet) {
				System.out.println("It wasn't possible to update this identifier: " + DOI.SCHEME + doiRow.getDoi());
			}
	...
	public void delete(String identifier) throws SQLException {
		...
		try {
			...
			if (!quiet) {
				System.out.println(
						"It was possible to delete this identifier: " + DOI.SCHEME + doiRow.getDoi() + " online.");
			}
		} catch (DOIIdentifierException ex) {
			// Identifier was not recognized as DOI.
			LOG.error("It wasn't possible to detect this identifier:  " + identifier + " Exceptions code:  "
					+ ex.codeToString(ex.getCode()), ex);

			if (!quiet) {
				System.out.println("It wasn't possible to detect this identifier: " + identifier);
			}
		} catch (IllegalArgumentException ex) {
			if (!quiet) {
				System.out.println("It wasn't possible to delete this identifier: " + DOI.SCHEME + doiRow.getDoi()
	 ...
	 *                   contains digits only we treat it as ItemID, if not we try
	 *                   to find a matching doi or a handle (in this order).
	 * @return The TableRow or null if the Object does not have a DOI.
	 * @throws SQLException             if database error
	 * @throws IllegalArgumentException If the identifier is null, an empty String
	 *                                  or specifies an DSpaceObject that is not an
	 *                                  item. We currently support DOIs for items
	 *                                  only, but this may change once...
	 * @throws IllegalStateException    If the identifier was a valid DOI that is
	 *                                  not stored in our database or if it is a
	 *                                  handle that is not bound to an DSpaceObject.
	 */
	public DOI resolveToDOI(String identifier)
		...
		try {
			...
			if (!quiet) {
				System.out.println("It wasn't possible to detect this identifier: " + DOI.SCHEME + doiRow.getDoi());
			}
		}
	...
	private void sendAlertMail(String action, DSpaceObject dso, String doi, String reason) throws IOException {
		...
		try {
			if (recipient != null) {
				...
				if (!quiet) {
					System.out.println("Email alert is sent.");
				}
			}
		} catch (Exception e) {
			LOG.warn("Unable to send email alert", e);
			if (!quiet) {
				System.out.println("Unable to send email alert.");
			}
		}
	}
