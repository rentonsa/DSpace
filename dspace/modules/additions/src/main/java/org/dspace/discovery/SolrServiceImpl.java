 ...
 * that cron.
 *
 * The SolrServiceImpl is registered as a Service in the ServiceManager via a
...
public class SolrServiceImpl implements SearchService, IndexingService {
	...
	/**
	 * If the handle for the "dso" already exists in the index, and the "dso" has a
	 * lastModified timestamp that is newer than the document in the index then it
	 * is updated, otherwise a new document is added.
	 *
		...
		try {
			...
			case Constants.ITEM:
				...
				if (item.isArchived() || item.isWithdrawn()) {
					...
					/**
					 * Make sure the item is not in the index if it is not in archive or withwrawn.
					 */
	...
	/**
	 * Iterates over all Items, Collections and Communities. And updates them in the
	 * index. Uses decaching to control memory footprint. Uses indexContent and
	 * isStale to check state of item in index.
	 *
	 * @param context the dspace context
	 */
	@Override
	 ...
	 * isStale to check state of item in index.
	 * <p>
	 * At first it may appear counterintuitive to have an IndexWriter/Reader opened
	 * and closed on each DSO. But this allows the UI processes to step in and
	 * attain a lock and write to the index even if other processes/jvms are running
	 * a reindex.
	 *
	 * @param context the dspace context
	 ...
	 * @throws SearchServiceException occurs when something went wrong with querying
	 *                                the solr server
	 */
	@Override
	...
	public void buildSpellCheck() throws SearchServiceException {
		try {
			...
			// Make sure to also log the exception since this command is usually run from a
			// crontab.
			log.error(e, e);
	...
	/**
	 * Is stale checks the lastModified time stamp in the database and the index to
	 * determine if the index is stale.
	 *
	 ...
	 * @throws SQLException           sql exception
	 * @throws IOException            io exception
	 * @throws SearchServiceException if something went wrong with querying the solr
	 *                                server
	 */
	protected boolean requiresIndexing(String handle, Date lastModified)
		...
		// Specify that we ONLY want the LAST_INDEXED_FIELD returned in the field list
		// (fl)
	...
	public String createLocationQueryForAdministrableItems(Context context) throws SQLException {
		...
		if (context.getCurrentUser() != null) {
			List<Group> groupList = EPersonServiceFactory.getInstance().getGroupService().allMemberGroups(context,
			...
			for (ResourcePolicy rp : collectionsPolicies) {
				Collection collection = ContentServiceFactory.getInstance().getCollectionService().find(context,
						rp.getdSpaceObject().getID());
			...
			if (CollectionUtils.isNotEmpty(communitiesPolicies) || CollectionUtils.isNotEmpty(allCollections)) {
				...
				for (int i = 0; i < communitiesPolicies.size(); i++) {
					...
					Community community = ContentServiceFactory.getInstance().getCommunityService().find(context,
							rp.getdSpaceObject().getID());
	...
	protected void buildDocument(Context context, Community community) throws SQLException, IOException {
		// Create Document
		SolrInputDocument doc = buildDocument(Constants.COMMUNITY, community.getID(), community.getHandle(), null);
	...
	protected void buildDocument(Context context, Collection collection) throws SQLException, IOException {
		...
		SolrInputDocument doc = buildDocument(Constants.COLLECTION, collection.getID(), collection.getHandle(),
				locations);
	...
	/**
	 * Add the metadata value of the community/collection to the solr document IF
	 * needed highlighting is added !
	 * 
	 * @param doc                       the solr document
	 * @param highlightedMetadataFields the list of metadata fields that CAN be
	 *                                  highlighted
	...
	protected void buildDocument(Context context, Item item) throws SQLException, IOException {
		...
		// Keep a list of our sort values which we added, sort values can only be added
		// once
		...
		try {
			...
			for (MetadataValue meta : mydc) {
				...
				int minConfidence = isAuthorityControlled ? metadataAuthorityService.getMinConfidence(metadataField)
						: Choices.CF_ACCEPTED;
					...
					boolean ignoreAuthority = DSpaceServicesFactory.getInstance().getConfigurationService()
							.getPropertyAsType("discovery.index.authority.ignore." + field,
									DSpaceServicesFactory.getInstance().getConfigurationService().getPropertyAsType(
											"discovery.index.authority.ignore", new Boolean(false)),
									true);
					if (!ignoreAuthority) {
						...
						if (!ignorePrefered) {
							...
							preferedLabel = choiceAuthorityService.getLabel(meta, meta.getLanguage());
						}
						...
						boolean ignoreVariants = DSpaceServicesFactory.getInstance().getConfigurationService()
								.getPropertyAsType("discovery.index.authority.ignore-variants." + field,
										...
										true);
						if (!ignoreVariants) {
							variants = choiceAuthorityService.getVariants(meta);
						}
					...
					for (DiscoverySearchFilter searchFilter : searchFilterConfigs) {
						...
						if (searchFilter.getFilterType().equals(DiscoverySearchFilterFacet.FILTER_TYPE_FACET)) {
							if (searchFilter.getType().equals(DiscoveryConfigurationParameters.TYPE_TEXT)) {
								...
								// We use a separator to split up the lowercase and regular case, this is needed
								// to get our filters in regular case
								...
								if (date != null) {
									...
									// Also save a sort value of this year, this is required for determining the
									// upper & lower bound year of our facet
	...
	/**
	 * Helper function to retrieve a date using a best guess of the potential date
	 * encodings on a field
	 *
	...
	protected SolrQuery resolveToSolrQuery(Context context, DiscoverQuery discoveryQuery,
		...
		if (0 < facetFields.size()) {
			...
			for (DiscoverFacetField facetFieldConfig : facetFields) {
				...
				// Setting the facet limit in this fashion ensures that each facet can have its
				// own max
		...
		if (solrQueryResponse != null) {
				...
				for (int i = 0; i < facetFields.size(); i++) {
					...
					if (facetValues != null) {
						if (facetFieldConfig.getType().equals(DiscoveryConfigurationParameters.TYPE_DATE)
							...
							// If we have a date & are sorting by value, ensure that the results are flipped
							// for a proper result
			...
			if (solrQueryResponse.getFacetQuery() != null) {
				...
				for (String facetQuery : sortedFacetQueries.keySet()) {
					// TODO: do not assume this, people may want to use it for other ends, use a
					// regex to make sure
					// We have a facet query, the values looks something like: dateissued.year:[1990
					// TO 2000] AND -2000
					// Prepare the string from {facet.field.name}:[startyear TO endyear] to
					// startyear - endyear
	...
	@Override
	public void indexContent(Context context, DSpaceObject dso, boolean force, boolean commit)
