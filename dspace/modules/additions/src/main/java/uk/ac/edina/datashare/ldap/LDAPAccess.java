...
public class LDAPAccess {
	...
	/**
	 * Initialise LDAP directory access. Private access to prevent instantiation.
	 */
	private LDAPAccess() {
	...
	/**
	 * Get a user's email address for a university user name. If more than one LDAP
	 * entry is found, the first email is returned.
	 * 
	...
	public User getUserDetailsForUun(String uun) {
		...
		if (user != null) {
			if (user.getEmail() == null || user.getFirstName() == null || user.getSurname() == null) {
	...
	/**
	 * Get first LDAP entry found for the given search string for a staff member.
	 * 
	...
	private User getResult(String searchStr, DirContext context) {
		...
		try {
			...
			try {
				// perform search
				searchResults = context.search(CONTEXT_NAME, searchStr, this.constraints);
			} catch (Exception ex) {
				log.warn("Problem doing LDAP search for " + searchStr + ". " + ex.getMessage());
			}
