 ...
 * 
 * 1) No License - ensure rights has been populated. 2) Open Data License -
 * ensure license has been accepted. 3) ODC Attribution License - ensure license
 * has been accepted. 4) Creative Commons - ensure license has been chosen.
 */
public class UserLicenseStep extends LicenseStep {
	/*
	 * (non-Javadoc)
	 * 
	 * @see org.dspace.submit.AbstractProcessingStep#doProcessing(org.dspace.core.
	 * Context, javax.servlet.http.HttpServletRequest,
	 * javax.servlet.http.HttpServletResponse, org.dspace.app.util.SubmissionInfo)
	 */
	@Override
	public int doProcessing(Context context, HttpServletRequest request, HttpServletResponse response,
		...
		if (ob != null) {
			DSpaceUtils.setUserLicenseType(context, subInfo, ob.toString());
		}
	 ...
	 */
	private int doRightsStatement(Context context, HttpServletRequest request, SubmissionInfo subInfo) {
		int retVal = STATUS_COMPLETE;
		String rightStatement = request.getParameter(Consts.RIGHTS_STATEMENT);
		String buttonPressed = Util.getSubmitButton(request, NEXT_BUTTON);

		try {
			if (buttonPressed.equals(NEXT_BUTTON) && rightStatement.length() == 0) {
				MetaDataUtil.setRights(context, subInfo, null);
				retVal = Consts.MANDITORY_FIELD;
			} else {
				// ok now populate the metadata
				MetaDataUtil.setRights(context, subInfo, rightStatement);
	 ...
	 * @return submission status.
	 */
	private int doCreativeCommons(Context context, HttpServletRequest request, SubmissionInfo subInfo) {
		int retVal = STATUS_COMPLETE;
		...
		try {
			...
			if (buttonPressed.equals(NEXT_BUTTON) && (!creativeCommonsService.hasLicense(context, item))) {
	 ...
	 * @return submission status.
	 */
	private int doOdcAttribution(Context context, HttpServletRequest request, SubmissionInfo subInfo) {
	 ...
	 * @return submission status.
	 */
	private int doCreativeCommonsBy(Context context, HttpServletRequest request, SubmissionInfo subInfo) {
	 ...
	 * @return submission status.
	 */
	private int doOpenDataCommons(Context context, HttpServletRequest request, SubmissionInfo subInfo) {
	 ...
	 * @return submission status.
	 */
	private int doOpenLicense(Context context, HttpServletRequest request, SubmissionInfo subInfo, String licenseFile) {
		int retVal = STATUS_COMPLETE;
		...
		if (buttonPressed.equals(NEXT_BUTTON)) {
			try {
				...
				if (licenseFile.equals(Consts.CREATIVE_COMMONS_BY_LICENCE)) {
					MetaDataUtil.setRights(context, subInfo, Consts.CREATIVE_COMMONS_BY_RIGHTS_STATEMENT);
				}

				// now insert license
				creativeCommonsService.setLicense(context, item, new FileInputStream(licenseFile), "text/plain");
			} catch (Exception ex) {
				throw new RuntimeException(ex);
			}
		}
	 ...
	 * 
	 * @see
	 * org.dspace.submit.AbstractProcessingStep#getNumberOfPages(javax.servlet.http.
	 * HttpServletRequest, org.dspace.app.util.SubmissionInfo)
	 */
	@Override
	...
	private class CreateCommonsStep extends org.dspace.submit.step.CCLicenseStep {
		public int doCreativeCommons(Context context, HttpServletRequest request, SubmissionInfo subInfo)
